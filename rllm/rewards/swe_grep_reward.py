...
class MULocBenchEvaluator:
    ...
    def extract_ground_truth(self, ground_truth: Union[Dict, List]) -> Dict[str, Set]:
        """Extract ground truth locations. Supports both old dict format and new list format."""
        gt_locations = {
            'file': set(),
            'class': set(),
            'function': set()
        ...
        # --- NEW FORMAT: List of location dictionaries ---
        if isinstance(ground_truth, list):
            for item in ground_truth:
                ...
                for cls in classes:
                    if cls:
                        gt_locations['class'].add(f"{file_path}::{cls}")
                ...
                for func in functions:
                    if func:
                        # If classes exist, create class::function combinations
                        if classes:
                            for cls in classes:
                                if cls:
                                    func_loc = f"{file_path}::{cls}::{func}"
                                    gt_locations['function'].add(func_loc)
                        else:
                            # Standalone function
                            func_loc = f"{file_path}::{func}"
                            gt_locations['function'].add(func_loc)
        ...
        # --- OLD FORMAT: Dict with patch information ---
        for file_info in ground_truth.get('files', []):
            file_path = file_info['path']
    ...
    def extract_predictions(self, predictions: List[Dict]) -> Dict[str, List]:
        """Extract predictions (new format). Logic remains unchanged."""
        pred_locations = {
            'file': [],
            'class': [],
            'function': []
    ...
    def evaluate_single_issue(self, ground_truth: Union[Dict, List], predictions: List[Dict], k: int = 5) -> Dict:
        gt_locations = self.extract_ground_truth(ground_truth)
        pred_locations = self.extract_predictions(predictions)
